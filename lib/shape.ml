...
let resolve_ref ~response ~db segments =
  let rec walk shape ~db_view i = function
    | [] -> Found shape
    | seg :: rest -> (
        match shape with
        (* index access ([0]) is invisible in the segment list, so any
           array is entered through its sampled element *)
        | Array_ (Some el) -> walk el ~db_view i (seg :: rest)
        | Array_ None -> Opaque
        | Scalar _ ->
        ...
        | Object_ fields -> (
            ...
            | Some next -> walk next ~db_view:next_db_view (i + 1) rest
            | None -> (
                if not db_view then
                  Missing
                    {
                  ...
                  | `Row row -> (
                      ...
                      | Some next -> walk next ~db_view:false (i + 1) rest
                      | None ->
                          Missing
                            {
                              index = i;
  ...
  in
  walk (envelope response) ~db_view:false 0 segments
...
let fields_at ~response ~db segments =
  match resolve_ref ~response ~db segments with
  | Found shape -> (
